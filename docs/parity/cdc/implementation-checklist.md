...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CDC-001 | registration RPC schema | `tunnelrpc/proto/tunnelrpc.capnp` | `RegistrationServer.registerConnection(auth: TunnelAuth, tunnelId: Data, connIndex: UInt8, options: ConnectionOptions) -> ConnectionResponse`. Schema IDs: RegistrationServer `@0xf71695ec7fe85497`, TunnelAuth `@0x9496331ab9cd463f`, ConnectionOptions `@0xb4bf9861fe035d04`, ConnectionResponse `@0xdbaa9d03d52b62dc` | cfdrs-cdc `registration.rs` | audited, partial | local tests | open gap | schema field compare, method signature tests, Cap'n Proto codec tests | critical | Full baseline-matching schema types: `TunnelAuth`, `ClientInfo` (nested), `ConnectionOptions` (full field set), `ConnectionDetails`, `ConnectionError` (retry semantics), `ConnectionResponse` (union enum). 11 tests. Wire encoding remains JSON. See [docs/parity/cdc/registration-rpc.md](registration-rpc.md) |
...
| CDC-003 | registration response contract | `ConnectionResponse` union: `error(ConnectionError)` or `connectionDetails(ConnectionDetails)`. `ConnectionError` has `cause`, `retryAfter` (Int64 ns), `shouldRetry` (Bool). `ConnectionDetails` has `uuid`, `locationName`, `tunnelIsRemotelyManaged` | success returns `ConnectionDetails`; error returns structured `ConnectionError` with retry semantics | cfdrs-cdc `registration.rs` | audited, partial | local tests | open gap | response golden tests, error retry-semantics tests, ConnectionError field tests | high | `ConnectionResponse` is now a union enum (Success/Error). `ConnectionError` has `retry_after_ns` (i64) and `should_retry` (bool). Wire encoding remains JSON. |
| CDC-004 | ClientInfo nesting and fields | `ClientInfo` struct: `clientId` (Data, 16-byte UUID), `features` (List(Text)), `version` (Text), `arch` (Text). Nested inside `ConnectionOptions.client` | registration sends client identity with UUID and capability list | cfdrs-cdc `registration.rs` | audited, partial | local tests | open gap | clientId UUID tests, features list tests, nesting shape tests | high | `ClientInfo` has `client_id` (UUID), `features` (Vec), `version`, `arch`; nested in `ConnectionOptions.client`. `for_current_platform()` constructor. |
| CDC-005 | ConnectionOptions full field set | `ConnectionOptions`: `client` (ClientInfo), `originLocalIp` (Data), `replaceExisting` (Bool), `compressionQuality` (UInt8), `numPreviousAttempts` (UInt8) | all fields sent to edge during registration | cfdrs-cdc `registration.rs` | audited, partial | local tests | open gap | field-level tests, default-value tests | high | Full field set: `client` (ClientInfo), `origin_local_ip`, `replace_existing`, `compression_quality`, `num_previous_attempts`. `for_current_platform()` constructor. |
| CDC-006 | feature flags sent during registration | `ConnectionOptions.Client.Features`: default set `allow_remote_config`, `serialized_headers`, `support_datagram_v2`, `support_quic_eof`, `management_logs`; selector-added: `support_datagram_v3_2`, `postquantum`; CLI-passthrough only: `quick_reconnects`; deprecated (filtered before send): `support_datagram_v3`, `support_datagram_v3_1` | capability list negotiates edge behavior at registration time | cfdrs-cdc `features.rs` | audited, partial | local tests | open gap | feature list tests, deprecated-feature filtering tests, selector logic tests | high | `features.rs` module with `default_feature_list()`, `dedup_and_filter()`, always-on/selector/deprecated categorization. 3 tests. `quick_reconnects` is selector-gated, matching Go behavior. |
...
| CDC-008 | updateLocalConfiguration RPC | `RegistrationServer.updateLocalConfiguration(config: Data)` | pushes tunnel config to edge on connIndex==0 when not remotely managed | cfdrs-cdc `registration.rs` | audited, partial | local tests | open gap | config push tests, connIndex==0 guard tests | medium | `UpdateLocalConfigurationRequest` type defined. RPC dispatch and connIndex guard not yet wired. |
| CDC-009 | SessionManager interface | `SessionManager.registerUdpSession()` and `unregisterUdpSession()` | UDP session lifecycle over Cap'n Proto RPC | cfdrs-cdc `registration.rs` | audited, partial | local tests | open gap | session registration tests, session cleanup tests | high | `RegisterUdpSessionRequest`, `RegisterUdpSessionResponse`, `UnregisterUdpSessionRequest` types defined. RPC dispatch not yet wired. |
| CDC-010 | ConfigurationManager interface | `ConfigurationManager.updateConfiguration(version: Int32, config: Data) -> UpdateConfigurationResponse` | remote edge pushes config updates to cloudflared | cfdrs-cdc `registration.rs` | audited, partial | local tests | open gap | config update tests, version tracking tests | medium | `UpdateConfigurationRequest`, `UpdateConfigurationResponse` types defined. RPC dispatch not yet wired. |
...
| CDC-012 | ConnectRequest wire framing | `tunnelrpc/pogs/quic_metadata_protocol.go` uses Cap'n Proto binary marshaling via `ToPogs()` | stream requests encoded as Cap'n Proto binary | cfdrs-cdc `stream_codec.rs` | audited, partial | local tests | open gap | frozen-fixture wire tests, binary format comparison, malformed-input tests | critical | Wire codec extracted to CDC-owned `stream_codec.rs`. `decode_connect_request`/`encode_connect_request` with big-endian binary format. 8 codec tests. `lifecycle.rs` delegates to CDC codec. Not Cap'n Proto. |
| CDC-013 | ConnectResponse schema and framing | `quic_metadata_protocol.capnp`: `ConnectResponse` with `error` (Text), `metadata` (List(Metadata)). Cap'n Proto binary encoding. | per-stream response shape back to edge | cfdrs-cdc `stream_codec.rs` | audited, partial | local tests | open gap | schema compare, response construction tests, wire encoding tests | high | `ConnectResponse` encode/decode in `stream_codec.rs`. Wire encoding for both request and response paths. Roundtrip tests. Not Cap'n Proto. |
| CDC-014 | metadata key conventions | `connection/quic_connection.go`, `connection/header.go`, and `tracing/tracing.go` | keys: `HttpMethod`, `HttpHost`, `HttpHeader:<name>`, `HttpStatus`, `FlowID` (defined in `quic_connection.go`), `cf-trace-id` (`TracerContextName` in `tracing/tracing.go`), `HttpHeader:Content-Length` | cfdrs-cdc `stream.rs` | audited, partial | local tests | open gap | metadata key inventory tests, accessor tests | medium | Rust defines matching constants. Accessors exist. Missing evidence that all edge-expected keys are handled. |
| CDC-015 | transport header serialization | `connection/header.go` | base64.RawStdEncoding pairs joined by `;` for `cf-cloudflared-request-headers` and `cf-cloudflared-response-headers`; JSON for `cf-cloudflared-response-meta` | cfdrs-cdc `stream_contract.rs` | audited, partial | local tests | open gap | header serialization roundtrip tests, base64 encoding tests | high | Header key constants defined: `REQUEST_USER_HEADERS`, `RESPONSE_USER_HEADERS`, `RESPONSE_META_HEADER`. Serialization/deserialization logic not yet implemented. |
| CDC-016 | ResponseMeta contract | `connection/header.go` | pre-generated JSON: `{"src":"origin"}`, `{"src":"cloudflared"}`, `{"src":"cloudflared","flow_rate_limited":true}` | cfdrs-cdc `stream_contract.rs` | audited, partial | local tests | open gap | response meta shape tests | medium | `RESPONSE_META_ORIGIN`, `RESPONSE_META_CLOUDFLARED`, `RESPONSE_META_CLOUDFLARED_FLOW_LIMITED` constants match frozen baseline JSON. |
| CDC-017 | control header stripping | `connection/header.go` `IsControlResponseHeader` | headers with prefixes `:`, `cf-int-`, `cf-cloudflared-`, `cf-proxy-` stripped from user-visible responses | cfdrs-cdc `stream_contract.rs` | audited, partial | local tests | open gap | control header detection tests, stripping tests | medium | `CONTROL_HEADER_PREFIXES` and `is_control_response_header()` match baseline prefixes (`:`, `cf-int-`, `cf-cloudflared-`, `cf-proxy-`). 2 tests. |
...
| ID | Feature group | Baseline source | Baseline behavior or contract | Rust owner now | Rust status now | Parity evidence status | Divergence status | Required tests | Priority | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CDC-019 | control stream lifecycle | `connection/control.go` `ControlStreamHandler` | open control stream → register → optionally send local config → monitor for graceful shutdown → unregister | current QUIC transport `lifecycle.rs` and `protocol.rs` | audited, partial | partial | open gap | lifecycle integration tests, stage transition tests | high | Rust reports `Registered`, `RegistrationComplete`, `IncomingStream`, `Unregistering`, `Disconnected`, `ConfigPushed` events. Missing: config push dispatch, graceful shutdown wiring, unregister dispatch. |
| CDC-020 | connection status events | `connection/event.go` | `Event` with Index, EventType (Disconnected=0, Connected=1, Reconnecting=2, SetURL=3, RegisteringTunnel=4, Unregistering=5), Location, Protocol, URL, EdgeAddress | current protocol `protocol.rs` | audited, partial | local tests | open gap | event type inventory tests, transition tests | medium | `ProtocolBridgeState` has: BridgeUnavailable, BridgeCreated, RegistrationSent, RegistrationObserved, Reconnecting, Unregistering, BridgeClosed. Extended with `Reconnecting` and `Unregistering` to approach Go event granularity. |
| CDC-021 | protocol negotiation | `connection/protocol.go` | Protocol enum: HTTP2=0, QUIC=1. TLS server names: `h2.cftunnel.com` (HTTP/2), `quic.cftunnel.com` (QUIC). QUIC ALPN: `argotunnel`. Fallback: QUIC→HTTP/2. | cfdrs-cdc `protocol.rs` and current QUIC transport `edge.rs` | audited, partial | local tests | open gap | protocol selection tests, SNI tests, ALPN tests, fallback tests | high | CDC-owned `protocol.rs` defines `EDGE_H2_TLS_SERVER_NAME`, `EDGE_QUIC_TLS_SERVER_NAME`, `EDGE_QUIC_ALPN`, stream signatures, and `determine_protocol()`. 7 tests. No HTTP/2 transport or fallback. |
| CDC-022 | edge discovery | `edgediscovery/` | SRV record `_v2-origintunneld._tcp.argotunnel.com`, DNS-over-TLS fallback (dial `1.1.1.1:853`, TLS serverName `cloudflare-dns.com`), priority+weight sorting via Go stdlib, region1+region2 redundancy | cfdrs-cdc `protocol.rs` and current QUIC transport `edge.rs` | audited, partial | local tests | open gap | SRV record tests, DoT fallback tests, region failover tests | high | CDC-owned `protocol.rs` defines `EDGE_SRV_REGION1`, `DOT_RESOLVER_ADDR`, `DOT_SERVER_NAME`, `RESOLVE_TTL_SECS`. Constants match baseline. Runtime still uses A/AAAA (no SRV or region2 fallback). |
...
CDC-owned wire codec in `cfdrs-cdc/stream_codec.rs` (request and response
encode/decode, 8 tests). Stream contract constants in
`cfdrs-cdc/stream_contract.rs` (metadata keys, header keys, response meta,
control header detection). Protocol constants in `cfdrs-cdc/protocol.rs`
(stream signatures, TLS server names, ALPN, edge discovery DNS, 7 tests).
...
QUIC transport with JSON registration, custom binary stream framing, and
correct ALPN `argotunnel` in `cfdrs-bin`. Protocol events extended with
`Reconnecting`, `Unregistering`, `Disconnected`, `ConfigPushed` in
`cfdrs-bin/protocol.rs`. Tunnel credentials loading in `cfdrs-shared`.

Missing: Cap'n Proto wire codec wrappers (registration and stream framing), RPC
dispatch wiring for `unregisterConnection`, `updateLocalConfiguration`,
`registerUdpSession`/`unregisterUdpSession`, and `updateConfiguration`,
graceful shutdown, management service, log streaming, all API client methods,
readiness/metrics/healthcheck response contracts, datagram V2/V3, transport
header serialization/deserialization logic, SRV-based edge discovery runtime,
region2 fallback, actual origin HTTP round-trip for `Http(url)` dispatch.
...
The generated bindings are available but not yet wired into the codec paths.
Registration still uses JSON (`serde_json`) and stream framing still uses
custom big-endian binary format. The next implementation step is to build
CDC-owned codec wrappers around the generated types.

Wire encoding evidence artifacts needed before claiming wire parity:

- Cap'n Proto codec wrappers for registration request and response
- Cap'n Proto codec wrappers for ConnectRequest and ConnectResponse
- frozen Go Cap'n Proto binary fixtures for roundtrip comparison
- ALPN handshake evidence from Go QUIC connection

The first two items are now unblocked by the dependency admission.
...
- **JSON registration encoding**: Rust uses JSON for registration
  request/response; Go uses Cap'n Proto binary. This is a temporary gap, not
  an intentional divergence. The Cloudflare edge speaks Cap'n Proto RPC
  natively on the control stream — JSON is not accepted. Cap'n Proto
  generated bindings are now available in cfdrs-cdc; codec wrappers to
  replace the JSON path are the next implementation step.

### Gap ranking by priority

Critical gaps:

- CDC-001: registration schema (types match, wire encoding still JSON)
- CDC-002: registration wire encoding (Cap'n Proto binary vs JSON)
- CDC-011: ConnectRequest schema (logical match, wire divergence)
- CDC-012: ConnectRequest wire framing (CDC-owned codec, still custom binary not Cap'n Proto)
...
- CDC-003: registration response (union enum and retry fields present, wire encoding gap)
- CDC-004: ClientInfo nesting (implemented, wire encoding gap)
- CDC-005: ConnectionOptions full field set (implemented, wire encoding gap)
- CDC-006: feature flags (code exists, runtime wiring pending)
- CDC-009: SessionManager (types exist, RPC dispatch pending)
- CDC-013: ConnectResponse (encode/decode exists, Cap'n Proto pending)
- CDC-015: transport header serialization (constants defined, logic pending)
- CDC-019: control stream lifecycle (events extended, dispatch pending)
- CDC-021: protocol negotiation (CDC-owned constants, no HTTP/2 fallback)
- CDC-022: edge discovery (CDC-owned constants, runtime SRV pending)
