...
registration types (`registration_codec.rs`); the runtime still sends JSON
for registration and uses a custom big-endian binary format for stream
framing. Wire integration is in progress.
...
`ConnectionEvent` struct, 16 tests).
...
Runtime still
uses JSON registration and custom binary stream framing in `cfdrs-bin`;
correct ALPN `argotunnel` derived from CDC constant in `cfdrs-bin`. Protocol events extended with
...
`updateLocalConfiguration`, `registerUdpSession`/`unregisterUdpSession`,
and `updateConfiguration`, graceful shutdown, management service, log
...
actual origin HTTP round-trip for `Http(url)` dispatch, runtime consumption
of CDC-owned `ConnectionEvent`/`Protocol`.
...
integration to replace the JSON registration path and wire the stream codec
in `lifecycle.rs` remains pending.
...
- runtime integration wiring to replace JSON registration in `lifecycle.rs`
...
- **Registration wire encoding**: Cap'n Proto binary codec implemented in
  ...
  six registration types (17 round-trip tests). Runtime integration to
  replace the JSON path in `lifecycle.rs` remains pending.
...
- CDC-001: registration schema (Cap'n Proto codec implemented, runtime integration pending)
- CDC-002: registration wire encoding (Cap'n Proto codec implemented, runtime integration pending)
- CDC-011: ConnectRequest schema (Cap'n Proto codec implemented, runtime integration pending)
- CDC-012: ConnectRequest wire framing (Cap'n Proto codec implemented, runtime integration pending)
- CDC-018: incoming stream round-trip (ConnectionType-aware dispatch, 19 origin tests, actual origin HTTP pending)
...
- CDC-003: registration response (union enum and retry fields present, Cap'n Proto codec implemented, runtime wiring pending)
- CDC-004: ClientInfo nesting (implemented with Cap'n Proto codec, runtime wiring pending)
- CDC-005: ConnectionOptions full field set (implemented with Cap'n Proto codec, runtime wiring pending)
...
- CDC-013: ConnectResponse (Cap'n Proto codec implemented, runtime integration pending)
- CDC-015: transport header serialization (serialize/deserialize implemented with base64, runtime wiring pending)
- CDC-019: control stream lifecycle (12 protocol tests covering baseline lifecycle order, dispatch pending)
