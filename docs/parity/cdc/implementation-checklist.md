...
- CDC-007: unregisterConnection RPC (partial — type surface present; RPC dispatch wiring pending)
- CDC-008: updateLocalConfiguration RPC (partial — codec and `send_local_configuration` wired; connIndex guard and full RPC dispatch wiring pending)
- CDC-009: SessionManager (partial — types + codec + 5 codec tests; RPC dispatch wiring pending)
- CDC-010: ConfigurationManager (partial — types + codec + 3 tests; RPC server dispatch wiring pending)
- CDC-019: control stream lifecycle (partial — 12 protocol tests, lifecycle events wired; `unregisterConnection` RPC dispatch wiring pending)
