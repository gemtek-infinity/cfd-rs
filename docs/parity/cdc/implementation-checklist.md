...
- CDC-018: incoming stream round-trip (closed — ConnectionType-aware dispatch, 23 origin tests incl. reachable-origin round-trip, origin HTTP wired via reqwest)
...
- CDC-022: edge discovery (CDC-owned constants + edge address types in `edge.rs` with 46 tests; runtime SRV resolver in cfdrs-bin `edge.rs` with hickory-resolver DoT fallback, Regions integration, 9 runtime tests)
