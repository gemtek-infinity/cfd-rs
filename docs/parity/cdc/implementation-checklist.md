...
| CDC-007 | unregisterConnection RPC | `RegistrationServer.unregisterConnection()` | graceful shutdown over control stream with configurable grace period | cfdrs-cdc `registration.rs`, `registration_codec.rs`, `rpc_dispatch.rs` | audited, parity-backed | local tests | closed | graceful shutdown tests, grace period tests | medium | `UnregisterConnectionRequest` type, `RegistrationClient::unregister_connection()` capnp-rpc client wrapper with local-dispatch test. RPC dispatch layer complete. Runtime wired: `send_unregister_connection()` in `lifecycle.rs` encodes via `encode_unregister_request()`, sends on control stream with `fin=true` (last message), flushes egress. Called during graceful shutdown after `Unregistering` event, before `teardown_session()`. Codec functions `encode_unregister_request()` and `decode_unregister_response()` in `registration_codec.rs` with 3 round-trip tests. Best-effort semantics: errors logged but do not abort teardown, matching Go baseline behavior. Wire path uses raw `capnp::serialize` consistent with all other control-stream operations (registration, config push). |
...
| ID | Feature group | Baseline source | Baseline behavior or contract | Rust owner now | Rust status now | Parity evidence status | Divergence status | Required tests | Priority | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CDC-033 | tunnel CRUD API | `cfapi/tunnel.go` and `client.go` | `CreateTunnel`, `GetTunnel`, `GetTunnelToken`, `DeleteTunnel`, `ListTunnels`, `ListActiveClients`, `CleanupConnections` to `/accounts/{accountTag}/cfd_tunnel/...` | cfdrs-cdc `api.rs`, `api_resources.rs`; cfdrs-bin `api_client.rs` | audited, parity-backed | local tests | none recorded | API request shape tests, response envelope tests, error mapping tests, client construction tests | critical | `CloudflareApiClient` trait with all 7 tunnel CRUD methods matching Go `TunnelClient`. `ReqwestApiClient` in cfdrs-bin with `reqwest::blocking` HTTP client, paginated `list_tunnels` via `fetch_paginated`, `TunnelFilter` query builder. `Tunnel`, `TunnelWithToken`, `TunnelConnection`, `ActiveClient`, `NewTunnel` resource types with serde matching Go JSON shape. 13+ parity tests. See [docs/parity/cdc/metrics-readiness-and-api.md](metrics-readiness-and-api.md) |
| CDC-034 | API response envelope | `cfapi/base_client.go` | JSON envelope: `{"success":true,"errors":[],"messages":[],"result":...,"result_info":{...}}`. Error mapping: 400→ErrBadRequest, 401/403→ErrUnauthorized, 404→ErrNotFound | cfdrs-cdc `api.rs`; cfdrs-bin `api_client.rs` | audited, parity-backed | local tests | none recorded | envelope parsing tests, error mapping tests, `parse_result` typed extraction, `check` success validation | critical | `ApiResponse` with `parse_result<T>()` and `check()` helpers. `ApiClientError` enum (Unauthorized, BadRequest, NotFound, NoSuccess, `TunnelNameConflict`, `Transport`). Status code mapping in `map_status_error`: 200/201→Ok, 400→BadRequest, 401/403→Unauthorized, 404→NotFound, 409→Conflict. 15+ parity tests including typed parse and envelope check. |
| CDC-035 | API auth and headers | `cfapi/base_client.go` | `Authorization: Bearer <token>`, `Accept: application/json;version=1`, `Content-Type: application/json`, timeout 15s, HTTP/2 enabled | cfdrs-cdc `api.rs`; cfdrs-bin `api_client.rs` | audited, parity-backed | local tests | none recorded | auth header tests, content-type tests, config URL builder tests, client construction tests | high | `AUTHORIZATION_BEARER_PREFIX`, `API_ACCEPT_HEADER`, `JSON_CONTENT_TYPE`, `DEFAULT_API_TIMEOUT` constants match Go. `ApiClientConfig` with URL builders for account/zone paths. `ReqwestApiClient::new()` sets default headers (Accept, Authorization Bearer) and timeout matching Go. `DEFAULT_API_BASE_URL` and `FED_API_BASE_URL` constants. 6+ parity tests including config URL builders. |
| CDC-036 | IP route API | `cfapi/ip_route.go` | `ListRoutes`, `AddRoute`, `DeleteRoute`, `GetByIP` to `/accounts/{accountTag}/teamnet/routes/...` | cfdrs-cdc `api.rs`, `api_resources.rs`; cfdrs-bin `api_client.rs` | audited, parity-backed | local tests | none recorded | route API tests, filter query tests, `IpRouteFilter` tests | high | `CloudflareApiClient` trait with `list_routes` (paginated), `add_route`, `delete_route`, `get_route_by_ip` matching Go `IPRouteClient`. `IpRouteFilter` with `to_query_pairs()` always including `tun_types=cfd_tunnel`. `Route`, `DetailedRoute`, `NewRoute` resource types with serde matching Go JSON shape. 4+ parity tests. |
| CDC-037 | virtual network API | `cfapi/virtual_network.go` | `CreateVirtualNetwork`, `ListVirtualNetworks`, `DeleteVirtualNetwork`, `UpdateVirtualNetwork` to `/accounts/{accountTag}/teamnet/virtual_networks/...` | cfdrs-cdc `api.rs`, `api_resources.rs`; cfdrs-bin `api_client.rs` | audited, parity-backed | local tests | none recorded | vnet API tests, `VnetFilter` tests | medium | `CloudflareApiClient` trait with `create_virtual_network`, `list_virtual_networks` (single page matching Go), `delete_virtual_network` (with force), `update_virtual_network` (PATCH). `VnetFilter` with `by_name` and `to_query_pairs()`. `VirtualNetwork`, `NewVirtualNetwork`, `UpdateVirtualNetwork` resource types with serde matching Go JSON shape including `is_default_network` rename. 4+ parity tests. |
| CDC-038 | management token API | `cfapi/client.go` `GetManagementToken` | `GetManagementToken(tunnelID, resource)` with resource: logs, admin, host_details | cfdrs-cdc `api.rs`, `api_resources.rs`; cfdrs-bin `api_client.rs` | audited, parity-backed | local tests | none recorded | token request tests, resource scope tests | high | `CloudflareApiClient::get_management_token()` method matching Go. `ManagementResource` enum (Logs=0, Admin=1, HostDetails=2) with `repr(u8)` matching Go iota. `ReqwestApiClient` POST to `/accounts/{a}/cfd_tunnel/{id}/management/{resource}`. 2+ parity tests including repr value verification. |
| CDC-039 | hostname routing API | `cfapi/hostname_route.go` `RouteTunnel` | `RouteTunnel(tunnelID, route)` to `/zones/{zoneTag}/tunnels/{tunnelID}/routes` | cfdrs-cdc `api.rs`, `api_resources.rs`; cfdrs-bin `api_client.rs` | audited, parity-backed | local tests | none recorded | route API tests, `HostnameRoute` enum tests, `HostnameRouteResult` summary tests | medium | `CloudflareApiClient::route_tunnel()` method matching Go. `HostnameRoute` enum (Dns/Lb) with custom `Serialize`. `HostnameRouteResult` enum with `success_summary()`. `DnsRouteRequest`, `LbRouteRequest`, `DnsRouteResult`, `LbRouteResult` types with serde matching Go JSON shape. PUT to `/zones/{z}/tunnels/{id}/routes`. 10+ parity tests. |
