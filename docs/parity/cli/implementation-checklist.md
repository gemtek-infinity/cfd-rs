...
| CLI-006 | update command | `cmd/cloudflared/updater/update.go` | `update` command with flags: `--beta`, `--force` (hidden), `--staging` (hidden), `--version`; returns exit code 11 if update occurred, exit code 10 on error, exit 0 if no update needed | current CLI surface | audited, parity-backed | local tests | closed | help capture, update-behavior tests, exit-code tests (exit 11 on success, exit 10 on error) | high | `update` now has real parse/help/dispatch parity: `update --version <VER>` is command-scoped instead of colliding with root `--version`, hidden `--force`/`--staging` still parse, `execute_update()` wires `WorkersUpdater` through `run_manual_update()`, and CLI exit codes now match the Go baseline (`0` no update, `10` failure, `11` updated). Evidence: 4 parse/help tests in `cfdrs-cli`, 4 behavioral command tests in `cfdrs-bin`, and the updater HTTP/apply tests in `cfdrs-his` |
...
| CLI-018 | tunnel diag | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel diag` creates diagnostic report from local cloudflared instance | current CLI surface | audited, parity-backed | local tests | closed | help capture, diagnostic-output tests | medium | `execute_tunnel_diag()` now builds `DiagnosticOptions`, auto-discovers metrics listeners across the Go port range when `--metrics` is absent, writes the Go-shaped ZIP bundle, and preserves baseline-facing CLI output semantics including `No instances found`, multi-instance listing, and non-fatal invalid-log-configuration reporting. Evidence: 1 parse-dispatch test in `cfdrs-cli`, 5 behavioral integration tests in `cfdrs-bin`, and ZIP/report tests in `cfdrs-his` |
...
| CLI-030 | usage failure behavior | `cmd/cloudflared/main.go`, `cmd/cloudflared/linux_service.go`, `cmd/cloudflared/tunnel/cmd.go`, `vendor/github.com/urfave/cli/v2/app.go`, `vendor/github.com/urfave/cli/v2/command.go` | bad flags print urfave/cli `Incorrect Usage.` help to stdout; Linux `runApp` discards the returned `app.Run()` error so parse failures exit 0; unknown top-level tokens bypass urfave/cli unknown-command handling and fall through to `tunnel.TunnelCommand(c)` | current CLI surface | audited, intentional divergence | local tests | intentional divergence | divergence note, stderr/stdout capture, exit-code matrix, unknown-command tests, bad-flag tests | high | Rust intentionally normalizes usage failures to a POSIX-style contract: `CliOutput::usage_failure()` exits 2 with stderr-only `error:` guidance, `--bogus` becomes `error: unknown flag: --bogus`, and unknown top-level tokens are rejected explicitly instead of surfacing Go's tunnel fallthrough message. Evidence: `cfdrs-bin` integration tests cover unknown flags, unknown commands, and stderr/stdout placement; `cfdrs-cli` unit tests lock the exit-code and stream contract. Checked-in captures in [`docs/parity/cli/captures/error-and-compat.txt`](captures/error-and-compat.txt) and [`docs/parity/cli/captures/rust-current-surface.txt`](captures/rust-current-surface.txt) show the Go-vs-Rust split directly. |
...
classic tunnels). Behavioral implementations now cover service management,
tunnel CRUD/run/ready/diag/route/vnet/ingress, tail token plus streaming,
management token/help, compatibility removals, and the admitted access
subtree boundaries. Remaining admitted CLI behavior work is the top-level
`update` command, which depends on the HIS updater rows. NArg validation at
...
CLI-030 (usage failure behavior) and CLI-031 (`validate`) are the
intentional divergences in the CLI ledger.
...
High gaps: none remaining in the admitted CLI surface.

Medium gaps: none remaining in the admitted CLI surface.
