...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CLI-001 | root invocation | `cmd/cloudflared/main.go` `action()` | empty invocation enters service mode via `handleServiceMode()`: creates config file watcher, initializes `FileManager`, creates `AppManager` and `AppService`, runs daemonically. Non-empty invocation without a recognized subcommand falls through to `tunnel.TunnelCommand(c)` (see CLI-008). Not help. | current CLI surface | audited, partial | partial | open gap | blackbox empty invocation capture, stdout and stderr capture, exit-code compare, service-mode behavior test, non-empty fallthrough test | critical | Rust dispatches empty invocation (`ServiceMode`) to `execute_startup_command()` which enters config discovery via `discover_config()` → runtime startup matching Go `handleServiceMode()` → `FindOrCreateConfigPath()` → `AppManager` flow. Integration test `empty_invocation_enters_config_discovery` verifies bare invocation enters config discovery path (no stub error). 2 unit tests: `service_mode_dispatches_to_startup` and `service_mode_with_config_dispatches_to_runtime`. Full HIS service-mode infrastructure (watcher: HIS-041, reload loop: HIS-042, service manager: HIS-043) is already wired in runtime via `spawn_config_watcher()` and signal bridge. Remaining gap: Go `handleServiceMode()` creates `AppManager` with `AppService` running `actionLoop()` — Rust runtime starts transport directly without the overwatch wrapper pattern; HIS-043 `ServiceManager<S>` already exists but is not yet composed as the top-level daemon loop |
...
| CLI-009 | tunnel login | `cmd/cloudflared/tunnel/login.go` | `tunnel login` generates cert via browser auth; also exposed as top-level `login` for backward compat (hidden at top level when built as subcommand); `--fedramp`/`-f` flag for FedRAMP support | current CLI surface | audited, partial | local tests | open gap | help capture, login-flow tests (browser auth is external), flag tests | high | Rust parses `tunnel login` and top-level `login`; `--fedramp`/`-f` flag parsed; NArg validation accepts 0 args; auth flow not implemented; 1 parse-dispatch test, 1 NArg test |
| CLI-010 | tunnel create | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel create NAME` creates a new tunnel; produces tunnel UUID and credentials file | current CLI surface | audited, partial | local tests | open gap | help capture, creation-flow tests, output-format tests | critical | Rust parses `tunnel create` and dispatches to stub; NArg validation requires exactly 1 arg matching Go baseline; 1 parse-dispatch test, 2 NArg tests (rejects 0, rejects 2+); creation flow depends on CDC-033/034 |
| CLI-011 | tunnel list | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel list` lists existing tunnels; supports filtering and sorting flags | current CLI surface | audited, partial | local tests | open gap | help capture, list-output tests, filter-flag tests | high | Rust parses `tunnel list` and dispatches to stub; NArg validation accepts 0 args; 1 parse-dispatch test, 1 NArg test; list flow depends on CDC-033/034 |
| CLI-012 | tunnel run | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel run [TUNNEL]` proxies local web server by running the given tunnel; named-tunnel flow requires credentials | current CLI surface | audited, partial | local tests | open gap | help capture, run invocation matrix, credential-resolution tests | critical | Rust parses `tunnel run` with credential flags; `execute_tunnel_run()` implements full Go `runCommand()` token precedence chain: `--token` > `--token-file` > positional > config (see CLI-032 for full evidence). NArg validation rejects >1 positional args with exact Go baseline error message, exit code 255. 2 parse-dispatch tests, 11 token/NArg unit tests (CLI-032), 3 NArg integration tests. Remaining: actual named-tunnel edge connection depends on CDC |
| CLI-013 | tunnel delete | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel delete TUNNEL` deletes existing tunnel by UUID or name | current CLI surface | audited, partial | local tests | open gap | help capture, delete-flow tests | high | Rust parses `tunnel delete` and dispatches to stub; NArg validation requires exactly 1 arg; 1 parse-dispatch test, 2 NArg tests; delete flow depends on CDC-033/034 |
| CLI-014 | tunnel cleanup | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel cleanup TUNNEL` cleans up tunnel connections; `--connector-id` flag to filter | current CLI surface | audited, partial | local tests | open gap | help capture, cleanup tests | medium | Rust parses `tunnel cleanup` and dispatches to stub; NArg validation requires exactly 1 arg; 1 parse-dispatch test, 2 NArg tests; cleanup flow depends on CDC-033/034 |
| CLI-015 | tunnel token | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel token TUNNEL` fetches credential token for existing tunnel by name or UUID | current CLI surface | audited, partial | local tests | open gap | help capture, token-output tests | high | Rust parses `tunnel token` and dispatches to stub; NArg validation requires exactly 1 arg; 1 parse-dispatch test, 2 NArg tests; token flow depends on CDC-033/034 |
| CLI-016 | tunnel info | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel info TUNNEL` lists details about active connectors | current CLI surface | audited, partial | local tests | open gap | help capture, info-output tests | medium | Rust parses `tunnel info` and dispatches to stub; NArg validation requires exactly 1 arg; 1 parse-dispatch test, 2 NArg tests |
...
| CLI-019 | tunnel route | `cmd/cloudflared/tunnel/subcommands.go` | `tunnel route` parent command with subcommands `dns`, `lb`, `ip`; `ip` has sub-subcommands `add`, `show`/`list`, `delete`, `get` | current CLI surface | audited, partial | local tests | open gap | help capture, per-subcommand tests | high | Rust parses nested route/dns/lb/ip/add/show/delete/get and dispatches to stubs; NArg validation: `dns`/`lb`/`ip add` require exactly 2 args, `ip show`/`delete`/`get` require exactly 1 arg, matching Go baseline; 4 parse-dispatch tests, 10 NArg tests across all sub-subcommands; route flow depends on CDC-036 |
| CLI-020 | tunnel vnet | `cmd/cloudflared/tunnel/vnets_subcommands.go` | `tunnel vnet` with subcommands `add` (with `--default`), `list`, `delete` (with `--force`), `update` (with `--name`, `--comment`) | current CLI surface | audited, partial | local tests | open gap | help capture, per-subcommand tests | medium | Rust parses vnet add/list/delete/update and dispatches to stubs; NArg validation: `add`/`delete`/`update` require exactly 1 arg, `list` accepts 0, matching Go baseline; 3 parse-dispatch tests, 6 NArg tests; vnet flow depends on CDC-037 |
...
| CLI-032 | run command (Rust alpha) | partial overlap with frozen `tunnel` and `tunnel run` | current Rust `run` enters QUIC transport core + Pingora proxy seam; only partially overlaps frozen `tunnel` root runnable behavior and `tunnel run` named-tunnel flow; must not be treated as CLI parity | current runtime + current CLI surface | audited, partial | local tests | open gap | command contract tests, compare against frozen `tunnel` root and `tunnel run` behavior | critical | `execute_tunnel_run()` implements the full Go `runCommand()` token precedence chain: `--token` > `--token-file` > positional arg > config `tunnel` field, matching exact Go error semantics. `resolve_run_token_string()` reads `--token` flag first, then `--token-file` (reads file, trims). Token validation via `TunnelToken::decode()` — invalid token exits 255 with `TUNNEL_TOKEN_INVALID_MSG` matching Go `"Provided Tunnel token is not valid."`. Token-file read error exits 255 with `TUNNEL_TOKEN_FILE_READ_ERROR_PREFIX` matching Go `"Failed to read token file: "`. No-identity exits 255 with `TUNNEL_RUN_IDENTITY_ERROR_MSG` matching Go `cliutil.UsageError()`. NArg validation rejects >1 positional args with exact Go baseline error message and exit code 255. 11 unit tests: `tunnel_run_with_valid_token_does_not_reject`, `tunnel_run_with_invalid_token_exits_255`, `tunnel_run_with_empty_token_falls_through`, `tunnel_run_with_token_file_reads_token`, `tunnel_run_with_invalid_token_file_exits_255`, `tunnel_run_token_flag_takes_precedence_over_token_file`, `tunnel_run_no_identity_exits_255`, `tunnel_run_allows_zero_positional_args`, `tunnel_run_rejects_extra_positional_args`, `tunnel_run_accepts_single_positional_arg`, `bare_run_and_tunnel_run_both_dispatch`. 3 NArg integration tests. Parse-layer tests: `--token-file` flag parsing, `--token` + `--token-file` coexistence, positional tunnel name capture, bare `run --token` equivalence with `tunnel run --token`. Remaining gap: actual tunnel run behavior (connect to edge, proxy traffic) is alpha-limited |
...
**Root invocation (CLI-001):** Partially resolved. Rust dispatches empty
invocation to `execute_startup_command()` which enters config discovery via
`discover_config()` → runtime startup, matching Go `handleServiceMode()`
→ `FindOrCreateConfigPath()` → `AppManager` flow at the CLI dispatch level.
Integration test `empty_invocation_enters_config_discovery` verifies this.
Remaining gap: Go composes `AppManager` + `AppService.actionLoop()` as
the daemon wrapper — Rust starts the runtime directly. HIS rows (HIS-042,
HIS-043) track the `ServiceManager<S>` + `ReloadActionLoop` pattern.
...
- CLI-001: service mode dispatch (enters config discovery → runtime; AppManager/actionLoop wrapper pattern remains)
- CLI-010: tunnel create (parsed, NArg validated — needs CDC implementation)
- CLI-012: tunnel run (token precedence chain complete — actual edge connection depends on CDC)
- CLI-032: run command reconciliation (token precedence chain complete — actual tunnel behavior alpha-limited)
