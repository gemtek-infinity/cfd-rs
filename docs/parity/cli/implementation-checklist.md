...
| CLI-023 | tail subtree | `cmd/cloudflared/tail/cmd.go` | `tail [TUNNEL-ID]` streams remote logs; flags: `--connector-id`, `--event` (filter: cloudflared/http/tcp/udp), `--level` (default `debug`), `--sample` (default 1.0), `--token` (env `TUNNEL_MANAGEMENT_TOKEN`), `--management-hostname` (hidden, default `management.argotunnel.com`), `--trace` (hidden), `--loglevel` (default `info`, env `TUNNEL_LOGLEVEL`), `--origincert` (env `TUNNEL_ORIGIN_CERT`), `--output` (json/default); hidden subcommand `token` gets management JWT | current CLI surface | audited, partial | baseline-backed tests | open gap | WebSocket transport tests, management token integration tests, output-format tests | high | CLI surface parity-backed: bare `tail` and hidden `token` subcommand parsed and dispatched explicitly; 2 parse-dispatch tests in `cfdrs-cli`, 4 integration tests in `cfdrs-bin` (`tail_bare_dispatches_to_stub`, `tail_token_dispatches_to_stub`, `management_bare_dispatches_to_stub`, `management_token_dispatches_to_stub`); behavioral implementations are stubs pending CDC-026 WebSocket transport and CDC-038 management token API. CDC owns log-streaming contract. See [docs/parity/cli/tail-and-management.md](tail-and-management.md) |
| CLI-024 | management subtree | `cmd/cloudflared/management/cmd.go` | `management` (hidden, category `Management`) with hidden subcommand `token`; token subcommand requires `--resource` (values: `logs`, `admin`, `host_details`), `--origincert`, `--loglevel`, `--output` (json/default) | current CLI surface | audited, partial | baseline-backed tests | open gap | management token invocation tests, resource-flag tests | medium | CLI surface parity-backed: hidden `management` command and hidden `token` subcommand parsed and dispatched explicitly; 2 parse-dispatch tests in `cfdrs-cli`, 4 integration tests shared with CLI-023 in `cfdrs-bin`; behavioral implementations are stubs pending CDC-038 management token API. See [docs/parity/cli/tail-and-management.md](tail-and-management.md) |
...
- CLI-023: tail subtree (CLI surface parity-backed — behavioral stubs pending CDC-026 WebSocket transport and CDC-038 management token API)
...
- CLI-024: management subtree (CLI surface parity-backed — behavioral stubs pending CDC-038 management token API)
