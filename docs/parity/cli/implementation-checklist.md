...
| ID | Feature group | Baseline source | Baseline behavior or contract | Rust owner now | Rust status now | Parity evidence status | Divergence status | Required tests | Priority | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CLI-022 | access subtree | `cmd/cloudflared/access/cmd.go` | `access` command (alias `forward`) with subcommands: `login` (browser auth), `curl` (JWT injection), `token` (JWT production), `tcp` (aliases `rdp`, `ssh`, `smb` for TCP/RDP/SSH/SMB proxy), `ssh-config` (print SSH config), `ssh-gen` (generate short-lived cert); `--fedramp` flag | current CLI surface | audited, partial | baseline-backed tests | open gap | per-subcommand behavior tests | high | CLI surface parity-backed: bare `access` and `forward` alias both show access help (exit 0), all 6 subcommands + 3 tcp aliases parsed and dispatched explicitly; 6 parse-dispatch tests in `cfdrs-cli`, 12 integration tests in `cfdrs-bin` covering help, alias equivalence, and per-subcommand dispatch; behavioral implementations are stubs pending CDC/HIS runtime infrastructure (browser auth, TCP proxy, JWT, SSH certs). Go baseline has no `NArg` constraints on access subcommands. See [docs/parity/cli/access-subtree.md](access-subtree.md) |
| CLI-023 | tail subtree | `cmd/cloudflared/tail/cmd.go` | `tail [TUNNEL-ID]` streams remote logs; flags: `--connector-id`, `--event` (filter: cloudflared/http/tcp/udp), `--level` (default `debug`), `--sample` (default 1.0), `--token` (env `TUNNEL_MANAGEMENT_TOKEN`), `--management-hostname` (hidden, default `management.argotunnel.com`), `--trace` (hidden), `--loglevel` (default `info`, env `TUNNEL_LOGLEVEL`), `--origincert` (env `TUNNEL_ORIGIN_CERT`), `--output` (json/default); hidden subcommand `token` gets management JWT | current CLI surface | audited, partial | baseline-backed tests | open gap | WebSocket client streaming loop not yet connected (dependency not admitted) | high | `tail token` fully wired: `build_client` → `get_management_token(Logs)` → JSON `{"token":"..."}` output, matching Go `managementTokenCommand`. `tail` bare: filter parsing (`parse_tail_filters` — level/event/sample with Go-matching validation), management URL building (`build_management_url` — token acquisition, FedRAMP hostname selection via `parse_management_token().is_fed()`, connector-id query param), output formatting (`format_log_line`, `format_log_json`) all implemented. 14 unit tests in `tail_management::tests`, 3 behavioral integration tests in `main_tests.rs`. WebSocket client dial + streaming loop requires `tungstenite` or `tokio-tungstenite` crate admission. CDC-026 types and CDC-038 API are closed dependencies. See [`crates/cfdrs-bin/src/tail_management.rs`](../../../crates/cfdrs-bin/src/tail_management.rs), [docs/parity/cli/tail-and-management.md](tail-and-management.md) |
| CLI-024 | management subtree | `cmd/cloudflared/management/cmd.go` | `management` (hidden, category `Management`) with hidden subcommand `token`; token subcommand requires `--resource` (values: `logs`, `admin`, `host_details`), `--origincert`, `--loglevel`, `--output` (json/default) | current CLI surface | audited, partial | baseline-backed tests | open gap | management bare still shows stub help | medium | `management token` fully wired: `parse_management_resource` (logs/admin/host_details) → `build_client` → `get_management_token` → JSON `{"token":"..."}` output, matching Go `tokenCommand`. `management` bare remains a stub (Go displays urfave/cli auto-generated help). 1 behavioral integration test for `management token` dispatch, 1 stub test for `management` bare. Resource parsing tested in unit tests. See [`crates/cfdrs-bin/src/tail_management.rs`](../../../crates/cfdrs-bin/src/tail_management.rs), [docs/parity/cli/tail-and-management.md](tail-and-management.md) |
...
- CLI-022: access subtree (CLI surface parity-backed — behavioral stubs pending CDC/HIS runtime)
- CLI-023: tail subtree (token subcommand and pre-WebSocket logic fully wired — WebSocket client streaming loop pending `tungstenite` crate admission)
...
- CLI-024: management subtree (management token fully wired — management bare still shows stub)
...
- CLI-024: `management` subtree (hidden) — `management token` fully wired; bare `management` still shows stub
