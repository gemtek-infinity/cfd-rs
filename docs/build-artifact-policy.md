...
- [.github/workflows/on-pr-push.yml](../.github/workflows/on-pr-push.yml) validates the workspace via conditional
  `validate-app` and `validate-tools` jobs, both driven through [`Justfile`](../Justfile) recipes
...
- PR CI validates the generic workspace via path-sensitive conditional jobs and [`Justfile`](../Justfile) recipes
- merge or manual preview artifact workflows run a single test gate before
  the lane-specific build matrix, also through [`Justfile`](../Justfile) recipes
