...
`just fmt` always means `cargo +nightly fmt --all`.

## Completion workflow
When finishing a task that touched Rust files, run these steps in order:
1. `just validate-app` and, if tooling changed, `just validate-tools` — fix any failures before proceeding
...
3. `just fmt`
4. Summary and doc reconciliation — summarize the change, check whether any governance, parity, or status docs need updating to reflect the work
